...
function logMatchingReport(
  ...
  return Effect.logInfo(
    chalk`${label} ${[...messages].join(chalk`{gray , }`)}`,
  );
}

function logMismatchingReport(
  groupReport: Report.Version.Group,
  messages: Set<string>,
) {
  const name = groupReport.name;
...
export function logOtherCommands() {
  return Effect.logInfo(
    [
      '',
      '  What next?',
      chalk`{dim -} {yellow synopkg list-mismatches} to see more detail about mismatching versions`,
      chalk`{dim -} {yellow synopkg fix-mismatches} to fix version mismatches automatically`,
      chalk`{dim -} {yellow synopkg format} to sort and prettify your package.json files`,
      chalk`{dim -} {yellow synopkg update} to choose updates from the npm registry`,
      chalk`{dim -} {yellow synopkg --help} for everything else`,
      '',
    ].join(EOL),
  );
}
