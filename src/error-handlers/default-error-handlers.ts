...
export const defaultErrorHandlers: ErrorHandlers = {
  ...
  DeprecatedTypesError(err) {
    ...
    return Effect.logError(
      [
        chalk.red(
          `Your synopkg config file contains values deprecated in ${url}`,
        ),
        chalk.red('  Dependency Types:', err.types),
        chalk.red(
          '  Docs: https://synopkg.github.io/synopkg/config/dependency-types',
        ),
      ].join(EOL),
    );
  },
  InvalidCustomTypeError(err) {
    return Effect.logError(
      [
        chalk.red('Your synopkg config file contains an invalid custom type'),
        chalk.red('  Error:', err.reason),
        chalk.red('  Config:', err.config),
        chalk.red(
          '  Docs: https://synopkg.github.io/synopkg/config/custom-types',
        ),
      ].join(EOL),
    );
  },
  ...
  SemverGroupConfigError(err) {
    return Effect.logError(
      [
        ...
        chalk.red('  Config:', err.config),
        chalk.red(
          '  Docs: https://synopkg.github.io/synopkg/config/semver-groups',
        ),
      ].join(EOL),
    );
  },
  VersionGroupConfigError(err) {
    return Effect.logError(
      [
        ...
        chalk.red('  Config:', err.config),
        chalk.red(
          '  Docs: https://synopkg.github.io/synopkg/config/version-groups',
        ),
      ].join(EOL),
    );
  },
