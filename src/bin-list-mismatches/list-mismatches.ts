...
/** Exported to be reused by `synopkg lint` */
...
> = {
  ...
  Unfixable(amount: number) {
    ...
    const msg = chalk`{red ${padStart(amount)} ${
      ICON.panic
    } can be fixed manually using} {blue synopkg prompt}`;
    return Effect.logInfo(msg);
  },
  Valid(amount: number) {
    if (amount === 0) {
  ...
  SameRangeMismatch(report: Report.SameRangeMismatch) {
    ...
    return Effect.logInfo(
      [
        ...
        chalk`  {gray use {blue synopkg prompt} to fix this manually}`,
      ].join(EOL),
    );
  },
  ...
  UnsupportedMismatch(report: Report.UnsupportedMismatch) {
    ...
    return Effect.logInfo(
      [
        ...
        chalk`  {red use {blue synopkg prompt} to fix this manually}`,
      ].join(EOL),
    );
  },
