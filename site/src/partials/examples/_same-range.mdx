...
Add a [Same Range Version Group](/synopkg/config/version-groups/same-range) which allows local packages installed in `devDependencies` or `peerDependencies` to use different semver ranges, as long as they all match the local package version.
