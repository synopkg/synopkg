...
Add a [Pinned Version Group](/synopkg/config/version-groups/pinned) so that local packages are always installed using `workspace:*` when they are used in `devDependencies`.
